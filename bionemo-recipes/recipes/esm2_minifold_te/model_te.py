...
class SequenceToPairTE(nn.Module):
    ...
    def __init__(self, sequence_state_dim, inner_dim, pairwise_state_dim):
        super().__init__()
        self.layernorm = te.LayerNorm(sequence_state_dim, eps=1e-5)
        self.proj = te.Linear(sequence_state_dim, inner_dim * 2, bias=True)
        self.o_proj = te.Linear(2 * inner_dim, pairwise_state_dim, bias=True)
...
class PairToSequenceTE(nn.Module):
    ...
    def __init__(self, c_z=128, c_s=1024, c_s_out=1024):
        super().__init__()
        self.s_z_norm = te.LayerNorm(c_z, eps=1e-5)
        self.s_z_fc1 = te.Linear(c_z, c_z)
        self.s_z_fc2 = te.Linear(c_z, c_z)
        self.combiner = te.Linear(2 * c_z + c_s, c_s_out)
...
class FoldingTrunkTE(nn.Module):
    ...
    def __init__(
        ...
        self.seq_to_pair = SequenceToPairTE(c_s, c_z // 2, c_z)
        self.projection = te.Linear(c_z * 3, c_z)
        self.recycle = te.Linear(disto_bins, c_z)
        self.miniformer = MiniFormerTE(c_z, blocks=num_layers)
        self.fc_out_1 = te.Linear(c_z, c_z)
        self.fc_out_2 = te.Linear(c_z, disto_bins)
