...
def create_te_checkpoint(output_dir: Path) -> Path:
    ...
    Follows the same workflow as export.py: convert weights, patch config.json
    with auto_map, and copy the modeling file so the checkpoint is self-contained
    and loadable with ``trust_remote_code=True``.
    """
    ...
    shutil.copy(THIS_DIR / "modeling_llama_te.py", te_dir / "modeling_llama_te.py")
