...
def test_token_packing_dataloader(tokenizer_path, tmp_path):
    """Test that the token packing dataloader works."""
    parquet_path = tmp_path / "token_packing_sequences.parquet"
    table = pa.table({"sequence": ["A" * 300, "C" * 280, "G" * 260, "T" * 240]})
    pq.write_table(table, parquet_path)
    ...
    assert batches[0]["input_ids"].ndim == 2
    assert batches[0]["labels"].ndim == 2
