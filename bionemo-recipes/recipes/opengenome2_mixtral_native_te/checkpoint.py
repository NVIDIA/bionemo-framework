...
def get_fsdp2_checkpoint_process_group(
    ...
    *,
    ...
    """Choose the DCP process group for FSDP2 checkpoints.

    Expert-parallel parameters are DTensors sharded across the EP mesh, so all
    participating ranks must join checkpoint save/load when EP is enabled.
    Returning None lets DCP use the default world process group.
    """
