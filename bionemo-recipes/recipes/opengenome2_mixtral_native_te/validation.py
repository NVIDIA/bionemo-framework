...
def run_validation(
    ...
    for _ in range(num_batches):
        ...
        try:
            ...
            logger.warning("Validation forward pass failed on rank %s: %s", dist_config.rank, exc)
            continue
    ...
    loss_tensor = torch.tensor(
        [total_loss, float(total_tokens), float(num_evaluated), total_weighted_loss],
        device=device,
    )
    ...
    if dist_config.rank == 0:
        logger.info(
            "[VAL] HF loss=%.4f (ppl=%.2f) | Megatron loss=%.4f (ppl=%.2f) | batches=%d tokens=%d",
            avg_loss,
            perplexity,
            megatron_style_loss,
            megatron_ppl,
