...
def get_cosine_annealing_schedule_with_warmup(
    ...
    """Cosine annealing scheduler with warmup and optional constant (plateau) phase.

    The learning rate schedule is:
    1. Warmup: linearly increase from 0 to max_lr over num_warmup_steps
    2. Constant (optional): hold at max_lr for num_constant_steps (plateau phase)
    3. Cosine decay: decay from max_lr to min_lr over num_decay_steps
    4. After all phases: remain at min_lr

    This matches Megatron/NeMo's CosineAnnealing scheduler with constant_steps support.

    Args:
        optimizer: The optimizer to schedule.
        num_warmup_steps: Number of warmup steps.
        num_decay_steps: Number of decay steps after warmup (and constant phase if used).
        ...
        num_constant_steps: Number of steps to hold at max_lr after warmup (plateau).
            Default 0 means no plateau phase. Set to e.g. 1024 to match Megatron behavior.
    ...
    def lr_lambda(current_step: int):
        ...
        # Phase 2: Constant (plateau at max_lr)
        constant_end = num_warmup_steps + num_constant_steps
        if num_constant_steps > 0 and current_step <= constant_end:
            # Stay at max_lr during constant phase
            return 1.0
        ...
        # Phase 4: Beyond all phases, stay at min_lr
        return min_lr_ratio
