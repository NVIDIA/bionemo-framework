...
# Uses custom gradient_debug to log per-layer gradient norms, weight norms,
# and zero-gradient fractions every step to wandb.
# (nvdlfw_inspect / quant_stats_config is incompatible with MXFP8.)
# Runs 1000 steps (54000 -> 55000).
...
echo "========================================="
echo "FL1 Qinit Gradient Debug — Resume from 54000"
...
echo "Starting gradient debug run — resuming from 54000, target 55000..."
python train_fsdp2.py --config-name L2_lingua_7b_mxfp8_fl1_qinit \
  ...
  logger.frequency=10 \
  gradient_debug.enabled=true \
  gradient_debug.log_every_n_steps=1 \
  wandb.name=${EXP_NAME} \
  wandb.id=${EXP_NAME} \
