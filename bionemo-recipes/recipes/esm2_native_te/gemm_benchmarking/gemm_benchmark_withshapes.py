...
def benchmark_te_fp8_prequantized(
    ...
    try:
        ...
        # Create BF16 tensors in the layout expected by tex.generic_gemm.
        #
        # Important: tex.generic_gemm has its own conventions for (A, B, transa, transb) and
        # expected output orientation. With the settings used below (transa=False, transb=True),
        # and with A shaped (K, M) and B shaped (K, N), TransformerEngine expects the output D
        # to be shaped (N, M) (note the swapped order). This is fine for benchmarking throughput
        # (FLOP count is still 2*M*N*K); it's just a layout convention.
        ...
        # Output buffer (see note above about expected (N, M) orientation)
...
def benchmark_te_fp4_prequantized(
    ...
    try:
        ...
        # Create BF16 tensors in the layout expected by tex.generic_gemm.
        # See FP8 pre-quantized path above for rationale (including expected D orientation).
        A = torch.randn(K, M, dtype=torch.bfloat16, device=device)
        B = torch.randn(K, N, dtype=torch.bfloat16, device=device)
