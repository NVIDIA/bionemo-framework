...
class MutationDataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        # Store and validate task type
        self.task = task
        assert task in ["mlm", "clm"], "Task must be either mlm or clm"
        ...
        self.causal = causal
        self.adjusted_context_length = context_length - MetadataConstants.MLM_TOK_ADJUST if task == "mlm" else context_length - MetadataConstants.CLM_TOK_ADJUST
        self.sequence_extractor = FastRefAlt(context_length=self.adjusted_context_length, causal=causal)
        self.process_item = process_item
        self.seed = seed
        self.extract_seq = extract_seq
        self.label_col = label_col
        self.organism_token = organism_token
...
class FastRefAlt:
    ...
    def __init__(self, context_length=None, nan_codon="NA", causal=False):
        """
        Initialize the FastRefAlt.

        Args:
            context_length (int, optional): The length of the output sequences. Defaults to None.
            causal (bool, optional): If True, use maximum left context (start from beginning) 
                                     instead of centering around the mutation. This is useful 
                                     for causal/autoregressive models. Defaults to False.
        """
        self.context_length = context_length
    ...
    def __call__(self, ref_cds, codon_position, ref_codon, alt_codon) -> Tuple[str, int, str, str]:
        """Get the subsequence described by the location.
        
        If causal=False: Centers the context window around the variant codon.
        If causal=True: Uses maximum left context, starting from position 0.
        ...
        if total_codons <= context_len_codons:
            # Sequence fits within context - use full sequence
            start_codon = 0
            end_codon = total_codons
        elif self.causal:
            # Causal mode: maximize left context, always start from 0
            # Include mutation position within the window
            start_codon = 0
            end_codon = min(context_len_codons, total_codons)
            
            # If mutation is beyond context window, shift window to include it
            if codon_position >= context_len_codons:
                # Shift window so mutation is at the last position
                end_codon = codon_position + 1
                start_codon = end_codon - context_len_codons
        else:
            # Centered mode: center around mutation
