...
def get_dummy_data_bshd_no_padding(tokenizer, seq_length: int = 64):
    """Get dummy data for the BSHD format without padding for tensor parallelism.

    For BSHD format with tensor parallelism, we use sequences of exactly the same length
    to avoid the complexity of handling attention masks across CP ranks.

    Args:
        tokenizer: The tokenizer to use.
        seq_length: The target sequence length (must be divisible by 2*tp_world_size).
...
@skip_in_ci
def test_tensor_parallel_equivalence_1process(recipe_path: Path):
    """Test that tensor parallelism works with 1 process, verifying results match non-distributed run."""
    cmd = [
        "torchrun",
        "--nproc_per_node=1",
...
@skip_in_ci
@requires_multi_gpu
def test_tensor_parallel_equivalence_2process(recipe_path: Path):
    """Test tensor parallel equivalence between 2 processes.

    In one instance, we run the model in non-distributed mode and in the other
    we run the model in distributed mode with tensor parallelism. We then compare
...
if __name__ == "__main__":
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        group_fsdp_cp = device_mesh[("ddp", "tp")]._flatten("dp_cp").get_group()
        ...
        # Set up tensor parallelism for each layer
        for i, transformer_layer in enumerate(model.module.model.layers):
            transformer_layer.set_tensor_parallel_group(tp_group)
        ...
        outputs_cp = model(**batch)
        loss_cp = outputs_cp.loss

        # Gather the losses from all cp ranks (collective operation - all ranks must participate)
        losses_list = [torch.zeros_like(outputs_cp.loss) for _ in range(tp_world_size)]
        torch.distributed.all_gather(losses_list, outputs_cp.loss, group=tp_group)

        if tp_rank == 0:
            average_tp_loss = torch.mean(torch.stack(losses_list))
        ...
        logits_list = [torch.zeros_like(logits_contiguous) for _ in range(tp_world_size)]
        torch.distributed.all_gather(logits_list, logits_contiguous, group=tp_group)
        ...
        if tp_rank == 0:
            ...
            for batch_idx in range(batch_size):
                for tp_idx, logits_shard in enumerate(logits_list):
                    # Determine which chunks this CP rank has
                    chunk_indices = [tp_idx, total_chunks - tp_idx - 1]
