...
def test_converted_model_roundtrip(tmp_path, input_data):
    ...
    assert isinstance(model_te.model, NVEsmModel)
    assert isinstance(model_te.model.encoder, NVEsmEncoder)
    assert isinstance(model_te.model.encoder.layers[0], TransformerLayer)
