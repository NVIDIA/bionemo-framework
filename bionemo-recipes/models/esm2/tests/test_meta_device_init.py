...
def verify_model_parameters_initialized_correctly(model: NVEsmForMaskedLM, atol=1e-3, rtol=1e-4):
    config = model.config
...
def test_cuda_fp8_init(fp8_recipe):
    ...
    verify_model_parameters_initialized_correctly(model, atol=1e-2)
