...
def run_benchmark(dataloader: Any, config: BenchmarkConfig) -> BenchmarkResult:
    ...
    def benchmark_iteration():
        ...
        prev_memory = psutil.Process(os.getpid()).memory_info().rss
        ...
        for num, batch in enumerate(dataloader):
            ...
            #print(f"Memory usage: {mem_info.rss / (1024 * 1024):.2f} MB (RSS)")
            print(f"Memory usage delta: {(mem_info.rss - prev_memory) / (1024 * 1024):.2f} MB")
            
            prev_memory = mem_info.rss
