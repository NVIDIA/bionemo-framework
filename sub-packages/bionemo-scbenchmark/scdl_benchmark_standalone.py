...
and reports key performance metrics including instantiation time, samples per second,
baseline memory, and peak memory.

No external BioNeMo benchmark framework required - everything is self-contained.
"""
...
def format_report(result, input_path: str, sampling_scheme: str, method: str = "SCDL") -> str:
    ...
    report.append(f"  Dataset on Disk:   {result.disk_size_mb:.1f} MB")
    report.append("")
...
def format_comparison_report(scdl_result, anndata_result, input_path: str, sampling_scheme: str) -> str:
    ...
    report.append(f"  Speedup:           {speedup:.2f}x faster with SCDL")
    report.append("")
    
    report.append("MEMORY COMPARISON:")
    report.append(f"  SCDL Peak:         {scdl_peak:.1f} MB")
    report.append(f"  AnnData Peak:      {anndata_peak:.1f} MB")
    report.append(f"  Memory Efficiency: {memory_ratio:.2f}x more memory with AnnData")
    report.append("")
    ...
    if speedup > 1:
        ...
        report.append(f"  AnnData provides {1/speedup:.1f}x throughput improvement")
    
    if memory_ratio > 1:
        report.append(f"  SCDL uses {memory_ratio:.1f}x less memory")
    else:
        report.append(f"  AnnData uses {1/memory_ratio:.1f}x less memory")
