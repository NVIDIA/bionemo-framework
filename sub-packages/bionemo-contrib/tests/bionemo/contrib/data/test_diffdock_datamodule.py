...
def test_datamodule_score_model_init(split, get_diffdock_score_model_heterodata,
                            create_datamodule_score_model):
    name_split = str(split).split('.')[1]
    (_, _, names) = get_diffdock_score_model_heterodata
    data_module, prefix_dir_tars_wds = create_datamodule_score_model
...
@pytest.mark.parametrize("split", [s for s in Split])
def test_datamodule_confidence_model_init(split, get_diffdock_confidence_model_heterodata,
                            create_datamodule_confidence_model):
    name_split = str(split).split('.')[1]
    (_, _, names) = get_diffdock_confidence_model_heterodata
    data_module, prefix_dir_tars_wds = create_datamodule_confidence_model
    assert data_module._sizes[split] == len(names[split]),\
        f"Wrong {split}-set size: expected {len(names[split])}"\
        f"but got {data_module._sizes[split]}"
    assert data_module._dirs_tars_wds[split] ==\
...
@pytest.mark.parametrize("split", [s for s in Split])
def test_datamodule_score_model_prepare_data(split, create_datamodule_score_model):
    data_module, _ = create_datamodule_score_model
...
@pytest.mark.parametrize("split", [s for s in Split])
def test_datamodule_confidence_model_prepare_data(split, create_datamodule_confidence_model):
    data_module, _ = create_datamodule_confidence_model
    # LightningDataModule.prepare_data() is supposed to be called from the main
    # process in a Lightning-managed multi-process context so we can call it in
    # a single process
    data_module.prepare_data()
...
@pytest.mark.parametrize("split", [s for s in Split])
def test_datamodule_score_model_setup_dataset(split, create_datamodule_score_model, create_another_datamodule_score_model):
    data_modules= [create_datamodule_score_model[0], create_another_datamodule_score_model[0]]
    lists_complex_name = []
    lists_pos_ligand = []
...
@pytest.mark.parametrize("split", [s for s in Split])
def test_datamodule_confidence_model_setup_dataset(split, create_datamodule_confidence_model, create_another_datamodule_confidence_model):
    data_modules= [create_datamodule_confidence_model[0], create_another_datamodule_confidence_model[0]]
    lists_complex_name = []
    lists_pos_ligand = []
    for m in data_modules:
        m.prepare_data()
...
@pytest.mark.parametrize("split", [s for s in Split])
def test_datamodule_score_model_setup_dataloader(split, create_datamodule_score_model, create_another_datamodule_score_model):
    data_modules= [create_datamodule_score_model[0], create_another_datamodule_score_model[0]]
    lists_complex_name = []
    lists_pos_ligand = []
    for m in data_modules:
...
@pytest.mark.parametrize("split", [s for s in Split])
def test_datamodule_confidence_model_setup_dataloader(split, create_datamodule_confidence_model, create_another_datamodule_confidence_model):
    data_modules= [create_datamodule_confidence_model[0], create_another_datamodule_confidence_model[0]]
    lists_complex_name = []
    lists_pos_ligand = []
    for m in data_modules:
