...
from bionemo.geneformer_t.data.temporal_dataset import TemporalGeneformerDataset
...
class TemporalGeneformerDataModule(SingleCellDataModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.tokenizer_vocab_path = Path(tokenizer_vocab_path)
        self.median_dict_path = Path(median_dict_path)
        ...
        self.preprocessor = GeneformerPreprocess(
            download_directory=self.data_path,
            medians_file_path=self.median_dict_path,
    ...
    def prepare_data(self):
        """Prepare data by running preprocessing."""
        logger.info("Preparing data for temporal Geneformer training...")

        # Run preprocessing to get tokenizer and median_dict
        result = self.preprocessor.preprocess()

        if isinstance(result, dict) and "tokenizer" in result and "median_dict" in result:
            self.tokenizer = result["tokenizer"]
            self.median_dict = result["median_dict"]
            logger.info("Preprocessing completed successfully")
        else:
            raise RuntimeError("Preprocessing failed - could not load tokenizer and median_dict")
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        """
        if self.tokenizer is None or self.median_dict is None:
            self.prepare_data()
    ...
    def _collate_fn(self, batch):
        ...
        for key in keys:
            ...
            elif key == "temporal_attention_mask":
                # Handle temporal attention mask if present
