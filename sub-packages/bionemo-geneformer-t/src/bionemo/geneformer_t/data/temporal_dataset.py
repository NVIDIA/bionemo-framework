...
class TemporalGeneformerDataset(Dataset):
    ...
    def _create_temporal_sequence(
        ...
        # Create temporal attention mask (prevents next cell self-attention)
        temporal_attention_mask = self._create_temporal_attention_mask(
            current_len=len(cls_token) + len(current_tokens), sep_len=1, next_len=len(next_tokens), total_len=seq_len
        )
        ...
        if seq_len < self.max_len:
            ...
            # Pad temporal attention mask
            padded_temporal_mask = torch.zeros((self.max_len, self.max_len), dtype=temporal_attention_mask.dtype)
            padded_temporal_mask[:seq_len, :seq_len] = temporal_attention_mask
            temporal_attention_mask = padded_temporal_mask
    ...
    def _create_temporal_attention_mask(
        self, current_len: int, sep_len: int, next_len: int, total_len: int
    ) -> torch.Tensor:
        """Create temporal attention mask preventing next cell self-attention.

        Attention pattern:
        - Current cell (including CLS) can attend to itself
        - SEP can attend to current cell
        - Next cell can attend to current cell and SEP, but NOT to itself

        Args:
            current_len: Length of current cell sequence (including CLS)
            sep_len: Length of separator (always 1)
        ...
        # Define regions
        current_end = current_len
        sep_end = current_end + sep_len
        next_end = total_len

        # Current cell can attend to itself (including CLS)
        mask[:current_end, :current_end] = True

        # SEP can attend to current cell
        mask[current_end:sep_end, :current_end] = True

        # Next cell can attend to current cell and SEP, but NOT itself
        mask[sep_end:next_end, :sep_end] = True
        # Next cell self-attention is explicitly disabled (remains False)
