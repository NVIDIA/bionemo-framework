...
class DistributedTrainingLauncherServer:
    """Distributed Training Launcher Server."""

    def __init__(self, host="localhost", port=6789, log_dir="/tmp/launcher_logs"):
        """Initialize the Distributed Training Launcher Server."""
        self.host = host
        self.port = port
        ...
        logging.basicConfig(
            ...
            handlers=[logging.FileHandler(self.log_dir / "launcher.log"), logging.StreamHandler()],
        )
    ...
    def launch_training(self, request):
        ...
        try:
            ...
            monitor_thread = threading.Thread(target=self.monitor_process, args=(process_id,))
    ...
    def get_logs(self, request):
        ...
        try:
            ...
            return {"status": "success", "logs": "".join(last_lines), "total_lines": len(all_lines)}
        except Exception as e:
