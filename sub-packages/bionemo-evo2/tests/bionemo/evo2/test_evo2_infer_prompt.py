...
def test_identical_prompts_should_be_identical(load_checkpoint, ckpt_name):
    ...
    with clean_parallel_state_context():
        response_prompt1 = run_inference(DEFAULT_PROMPT, checkpoint_path, InferCofig())
        response_prompt2 = run_inference(DEFAULT_PROMPT, checkpoint_path, InferCofig())   
...
def test_different_prompts_too_similar(load_checkpoint, ckpt_name):
    ...
    with clean_parallel_state_context():
        response_prompt1 = run_inference(DEFAULT_PROMPT, checkpoint_path, InferCofig())
        response_prompt2 = run_inference(ANIMAL_PROMPT, checkpoint_path, InferCofig())
