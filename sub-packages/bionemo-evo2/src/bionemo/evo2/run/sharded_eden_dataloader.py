...
class ShardedEdenDataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.window_min_length_threshold = (
    ...
    def build(
        ...
        if not dist.is_initialized() or dist.get_rank() == 0:
            print(
                "Creating datasets from pre-computed, split-specific window databases."
            )
...
class ShardedEdenDataset(Dataset):
    ...
    Assumes that the window_db_path points to a database pre-computed for a specific
    data split (e.g., train, validation, or test).
    """
    ...
    def __init__(
        ...
        if (
            not hasattr(ShardedEdenDataset, "_position_ids")
            or ShardedEdenDataset._position_ids.size(0) != seq_length
        ):
            ShardedEdenDataset._position_ids = torch.arange(
    ...
    def _open_all_sequence_dbs(self):
        """
        Open all sequence database files ahead of time and attach the window
        database for efficient cross-database queries.
        """
        self.db_connections = {}
        if not dist.is_initialized() or dist.get_rank() == 0:
            print(
                f"Pre-opening {len(self.sample_db_mapping)} sequence database files..."
            )
        ...
        for sample_id, db_path in self.sample_db_mapping.items():
            try:
                ...
                logging.error(
                    f"Failed to open/attach database for sample {sample_id} at {db_path}: {e}"
                )
                raise
    ...
    def _validate_and_setup_db(self):
        """
        Connects to the window database, validates its metadata,
        and computes the length of the dataset for the current split.
        """
        self.window_db_conn = sqlite3.connect(
            f"file:{self.window_db_path}?mode=ro", uri=True
        )
        ...
        try:
            ...
            if "window_size" not in db_meta or "stride" not in db_meta:
                raise ValueError(
                    "Database metadata is missing 'window_size' or 'stride' keys."
                )
            ...
            if (
                self.window_min_length_threshold
                and self.window_min_length_threshold > 0
            ):
        ...
        if not dist.is_initialized() or dist.get_rank() == 0:
            print(
                f"Found {self.length} windows for {self.split} split in {self.window_db_path}."
            )
    ...
    def _prepare_control_tags(self):
        ...
        unique_sequence_ids = [
            ...
            for row in cursor.execute(
                "SELECT DISTINCT sequence_id FROM window_mappings"
            )
        ]
        ...
        for seq_id in unique_sequence_ids:
            ...
            self.ctrl_ids_map[seq_id] = self.tokenizer.text_to_ids(
                f"<ctrl_{ctrl_name.lower()}>"
            )
    ...
    def _get_db_connection(self, sample_id: str) -> sqlite3.Connection:
        ...
        if conn is None:
            ...
            raise ValueError(
                f"No pre-opened SQLite connection found for sample {sample_id}"
            )
        return conn
    ...
    def __getitem__(self, idx: np.int64) -> Dict[str, torch.Tensor]:
        ...
        if res is None:
            ...
            current_dbs = self.window_db_conn.execute(
        ...
        # Build token window
        ctrl_ids = (
            self.ctrl_ids_map.get(sequence_id, []) if self.use_control_tags else []
        )
        ...
        # ------------------------------------------------------------------
        subseq_query = f"SELECT substr({SEQUENCE_COLUMN_NAME}, ?, ?) FROM sequences WHERE {SEQUENCE_ID_COLUMN_NAME} = ?"
        ...
        if result is None or result[0] is None:
            raise ValueError(
                f"Sequence ID {sequence_id} not found in database for sample {sample_id}"
            )
    ...
    def _init_window_logger(self, log_dir: Optional[str] = None):
        ...
        csv_path = (
            base_dir / f"window_access_{split_tag}_rank{rank}_{log_uuid[:8]}.csv"
        ).resolve()
...
def compute_num_windows(
    seq_len: int, window_size: int = 8192, stride: int = 7992
) -> int:
...
def precompute_window_database(
    ...
    # Validate columns
    if (
        SEQUENCE_ID_COLUMN_NAME not in df.columns
        or SEQUENCE_LENGTH_COLUMN_NAME not in df.columns
    ):
        raise ValueError(
    ...
    for seq_id, seq_len in df.select(
        [SEQUENCE_ID_COLUMN_NAME, SEQUENCE_LENGTH_COLUMN_NAME]
    ).iter_rows():
        ...
        if len(batch) >= batch_size:
            ...
            batch = []
            print(
                f"Processed {global_window_idx} windows... (skipped {skipped_windows})"
            )
    ...
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_sequence_id ON window_mappings(sequence_id)"
    )
    ...
    print(
        f"Finished. Found {total_sequences} sequences and {global_window_idx} total windows."
    )
    if window_min_length_threshold and skipped_windows > 0:
        print(
            f"Skipped {skipped_windows} windows due to window_min_length_threshold={window_min_length_threshold}."
        )

def main():
    """CLI for sharded Eden dataloader utilities."""
    parser = argparse.ArgumentParser(
        description="Utilities for sharded Eden dataloader: precompute window mappings."
    )
    subparsers = parser.add_subparsers(dest="command", help="Available commands")

    # Precompute subcommand
    precompute_parser = subparsers.add_parser(
        "precompute", help="Pre-compute window mappings from a Parquet file"
    )
    ...
    precompute_parser.add_argument(
        "output_window_db", type=str, help="Path to output window mapping database"
    )
    precompute_parser.add_argument(
        "--window-size", type=int, default=8192, help="Window size (default: 8192)"
    )
    precompute_parser.add_argument(
        "--stride",
        type=int,
    ...
    precompute_parser.add_argument(
        ...
        default=0,
        help=(
            "If > 0, skip sequences shorter than this length when precomputing windows. "
