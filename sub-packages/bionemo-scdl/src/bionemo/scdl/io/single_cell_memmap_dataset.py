...
from bionemo.scdl.schema.headerutil import build_feature_index_info
...
class FileNames(str, Enum):
    ...
    FEATURES = "features"  # For backward compatibility
...
class SingleCellMemMapDataset(SingleCellRowDataset):
    ...
    def __init__(
        ...
        Args:
            ...
            var_feature_index_name: The name of the var features if the features are only stored in features_df.index.values
    ...
    def _extract_features(self, features_df, feature_index_name):
        if features_df.columns.size > 0:
            return {col: np.array(features_df[col].values) for col in features_df.columns}
        elif features_df.index.size > 0:
            return {feature_index_name: features_df.index.values}
        return {}
    ...
    def load_h5ad(
        ...
        if file_size_MB < self.paginated_load_cutoff:
            var_features_df, obs_features_df, num_rows, _ = self.regular_load_h5ad(anndata_path)
        else:
            var_features_df, obs_features_df, num_rows, _ = self.paginated_load_h5ad(anndata_path)

        var_features = self._extract_features(var_features_df, self.var_feature_index_name)
        self._var_feature_index.append_features(n_obs=num_rows, features=var_features, label=anndata_path)

        obs_features = self._extract_features(obs_features_df, self.obs_feature_index_name)
    ...
    def _write_header(self):
        ...
        # Populate FeatureIndexInfo entries for row and column feature index directories
        indexes: List[FeatureIndexInfo] = []

        # Row features entry
        var_info = build_feature_index_info(self._var_feature_index, FileNames.VAR_FEATURES.value)
        if var_info is not None:
            indexes.append(var_info)

        # Column features entry
        obs_info = build_feature_index_info(self._obs_feature_index, FileNames.OBS_FEATURES.value)
        if obs_info is not None:
            indexes.append(obs_info)
