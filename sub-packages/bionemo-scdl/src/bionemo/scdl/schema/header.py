...
class ArrayDType(IntEnum):
    ...
    def from_numpy_dtype(cls, dtype) -> 'ArrayDType':
        ...
        elif dtype_str.startswith('<i') or dtype_str.startswith('>i') or dtype_str.startswith('<u') or dtype_str.startswith('>u'):
            if '1' in dtype_str:
...
class SCDLHeader:
    ...
    def validate(self) -> None:
        ...
        if self.version.major > current_version.major:
            raise HeaderSerializationError(
                f"Unsupported version: {self.version} > {current_version}"
            )
