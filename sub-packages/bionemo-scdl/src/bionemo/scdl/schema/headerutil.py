...
class BinaryHeaderCodec:
    ...
    def pack_string(self, value: str, max_length: int = None) -> bytes:
        """
        ...
        if max_length is not None and length > max_length:
            raise HeaderSerializationError(
                f"String too long: {length} bytes > {max_length} bytes limit"
            )
        
        return self.pack_uint32(length) + encoded_string

    def unpack_string(self, data: bytes, max_length: int = None) -> Tuple[str, int]:
        """
        ...
        if len(data) < 4 + length:
            raise HeaderSerializationError(
                f"Insufficient data for string: need {4 + length} bytes, got {len(data)}"
            )
    ...
    def pack_fixed_string(self, value: str, size: int, padding: bytes = b'\x00') -> bytes:
        ...
        if len(encoded) > size:
            raise HeaderSerializationError(
                f"String too long: {len(encoded)} bytes > {size} bytes field size"
            )
    ...
    def unpack_fixed_string(self, data: bytes, size: int, padding: bytes = b'\x00') -> str:
        ...
        if len(data) < size:
            raise HeaderSerializationError(
                f"Insufficient data: need {size} bytes, got {len(data)}"
            )
    ...
    def _validate_uint_range(self, value: int, min_val: int, max_val: int, type_name: str) -> None:
        ...
        if value < min_val or value > max_val:
            raise HeaderSerializationError(
                f"{type_name} value {value} out of range [{min_val}, {max_val}]"
            )
        ...
        for field_type, field_size in field_specs:
            if field_type == 'fixed_string':
                if not isinstance(field_size, int) or field_size <= 0:
                    raise HeaderSerializationError(
                        f"fixed_string requires positive integer size, got {field_size}"
                    )
