...
import numpy as np
import torch
from nemo.collections.llm.gpt.data.utils import _get_samples_mapping, handle_index
...
class MappedDataset(Dataset, ABC):
    ...
    def get_idx(self, idx):
        # Handle EpochIndex objects from MultiEpochDatasetResampler
        from bionemo.core.data.multi_epoch_dataset import EpochIndex
        if isinstance(idx, EpochIndex):
            # Pass through EpochIndex to the underlying dataset
            # but map the idx part through our sample mapping
            mapped_idx = self.sample_mapping[handle_index(self, idx.idx)]
            return EpochIndex(idx=mapped_idx, epoch=idx.epoch)
        else:
...
class ResamplingMappedDataset(MappedDataset):
    ...
    def create_sample_mapping(self, dataset: Dataset, num_samples: int) -> Sequence[int]:
        ...
        if index_mapping_type == "online":
            ...
            samples_mapping = _get_samples_mapping(
