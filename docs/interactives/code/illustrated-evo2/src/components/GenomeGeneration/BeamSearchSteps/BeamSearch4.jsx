...
const SCALE = 0.85; // shrink by 15%

const BeamSearch4 = ({ data, width = 1050, height = 190 }) => {
  const margin = { top: 0, right: 100, bottom: 0, left: 55 };
  const innerWidth = width - margin.left - margin.right;
  const innerHeight = height - margin.top - margin.bottom;

  // Shrunk node sizes
  const NODE_SIZE = 20 * SCALE;
  const BLOCK_WIDTH = 22 * SCALE;
  ...
  const getNodeWidth = node => {
    ...
    return NODE_SIZE; // Fixed width for extension nodes
  };
  ...
  const createTransformedData = () => {
    ...
    root.children[0].children = [
      createHypothesisNode('AA', true, [
        createExtensionNode('A', -1.39, false),
        createExtensionNode('G', -0.4, true), // Selected B from top row
        createExtensionNode('C', -1.39, false),
      ]),
    ];

    // Add B hypothesis after B extension (inactive path)
    // root.children[1].children = [
    //   createHypothesisNode('AG', false, [])
    // ];

    // Add C hypothesis after C extension
    root.children[2].children = [
      createHypothesisNode('AC', true, [
        createExtensionNode('T', -0.97, false),
        createExtensionNode('G', -0.97, false),
        createExtensionNode('C', -0.84, true), // Selected C from bottom row
      ]),
    ];

    // Continue building - AB hypothesis (from A->B)
    root.children[0].children[0].children[1].children = [
      createHypothesisNode('AAG', true, [
        createExtensionNode('A', -1.1, false),
        createExtensionNode('G', -0.9, true), // Selected B
        createExtensionNode('T', -0.95, true), // Also selected C from top row at T=2!
      ]),
    ];

    // Step 2
    // CC hypothesis (from C->C)
    root.children[2].children[0].children[2].children = [
      createHypothesisNode('ACC', false, [
        // This path is not active at T=2
        createExtensionNode('A', -1.5, false),
        createExtensionNode('G', -1.35, false),
        createExtensionNode('T', -1.4, false),
      ]),
    ];

    // Step 3
    // ABB hypothesis (from AB->B)
    root.children[0].children[0].children[1].children[0].children[1].children =
      [
        createHypothesisNode('AAGG', true, [
          createExtensionNode('A', -1.6, true), // Selected A
          createExtensionNode('T', -1.2, false),
          createExtensionNode('C', -1.6, false),
        ]),
      ];

    // ABC hypothesis (from AB->C) - This is the second selected path at T=2
    root.children[0].children[0].children[1].children[0].children[2].children =
      [
        createHypothesisNode('AAGT', true, [
          createExtensionNode('T', -1.1, false),
          createExtensionNode('G', -1.35, false),
          createExtensionNode('C', -1.25, false),
        ]),
      ];

    // Step 4
    // Final level - ABBA hypothesis (from ABB->A)
    root.children[0].children[0].children[1].children[0].children[1].children[0].children[0].children =
      [createHypothesisNode('AAGGT', true, [])];
  ...
  const { root, links } = useMemo(() => {
    ...
    for (
      ...
      if (nodesByDepth[depth]) {
        ...
        const levelSpacing = depth % 2 === 0 ? 80 * SCALE : 50 * SCALE; // More space after hypothesis nodes
  ...
  const renderLinkPath = link => {
    ...
    const nonHypothesisPadding = 10 * SCALE;
