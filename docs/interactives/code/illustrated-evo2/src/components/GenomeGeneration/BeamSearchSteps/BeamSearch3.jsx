...
const BeamSearch3 = ({ data, width = 1050, height = 180 }) => {
  const margin = { top: 0, right: 100, bottom: 0, left: 155 };
  const innerWidth = width - margin.left - margin.right;
  const innerHeight = height - margin.top - margin.bottom;

  // Scaled constants
  const SCALED_NODE_SIZE = 20 * SCALE;
  const SCALED_BLOCK_WIDTH = 22 * SCALE;
  const SCALED_SPACING = 2 * SCALE;

  const DNASequence = ({ sequence = 'ATCGA' }) => {
    const blockWidth = SCALED_BLOCK_WIDTH;
    const spacing = SCALED_SPACING;
    const nodeSize = SCALED_NODE_SIZE;
  ...
  const getNodeWidth = node => {
    ...
    return SCALED_NODE_SIZE; // Fixed width for extension nodes
  };
