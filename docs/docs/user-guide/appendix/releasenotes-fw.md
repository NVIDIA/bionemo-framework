...
* [NGC CLI](https://org.ngc.nvidia.com/setup/installers/cli) has been removed from the release container. If users download models from inside the container (via e.g. `download_artifacts.py` or `bionemo download`), the NGC CLI will be auto-installed to pull the models from NGC.
